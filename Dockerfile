...
RUN --mount=type=cache,target=/var/lib/apt/lists --mount=type=cache,target=/var/cache/apt/archives \
 ...
 && echo "comment: --add-dynamic-module=../ngx_http_geoip2_module"  \
 && make \
 && make install \
 && install -m 0755 -o $NGINX_USER -g $NGINX_USER -d /usr/share/nginx/html/.well-known/acme-challenge \
 && install -m 0755 -o $NGINX_USER -g $NGINX_USER -d /usr/share/nginx/html/.well-known/pki-validation \
